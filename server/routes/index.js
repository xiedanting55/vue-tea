...
router.get('/api/goods/id', function (req, res, next) {
  connection.query(`select * from goods_list where id=${req.query.id}`, function (error, results) {
    if (error) throw error
    res.json({
      code: 0,
      data: results[0]
...
router.get('/api/goods/shopList', (req, res, next) => {
  ...
  connection.query(`select * from goods_list where name like '%${name}%' order by ${orderName} ${order}`, (error, results) => {
    res.send({
      code: 0,
      data: results
    })
